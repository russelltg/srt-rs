...
use super::{
    ...
    ConnInitSettings, ConnInitSettings, ConnectError, ConnectError, ConnectionReject,
    ConnectionResult, SeqNumber,
};
use ConnectError::*;
use ConnectState::*;
    ...
    fn wait_for_conclusion(
        ...
        match (info.shake_type, info.info.version(), from) {
            (ShakeType::Conclusion, 5, from) if from == self.remote => {
                let settings =
                    match initiator.finish_hsv5_initiation(&info, self.init_send_seq_num, from) {
                        Ok(s) => s,
