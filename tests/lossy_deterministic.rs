...
use srt::{
    ...
    ConnectionSettings, Packet, SrtCongestCtrl,
};
use std::{
...
fn do_lossy_test(seed: u64, count: usize) {
    ...
    let mut sendr = Sender::new(s1, Handshake::Connector, SrtCongestCtrl);
    ...
    loop {
        if Some(current_time) == next_send_time {
            sendr.handle_data((current_time, Bytes::from(next_packet_id.to_string())));
