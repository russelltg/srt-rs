...
async fn invalid_packets() {
    let sender = async {
        let mut sender = SrtSocketBuilder::new_connect("127.0.0.1:8876").local_port(8877).connect().await.unwrap();
        

        let mut counting_stream = stream::iter(0..100)
            .zip(interval(Duration::from_millis(1)))
            .map(|(i, _)| Bytes::from(i.to_string())).map(|b| Ok((Instant::now(), b)));

        sender.send_all(&mut counting_stream).await.unwrap();
        sender.close().await.unwrap();
    };
    let recvr = async {
        let mut recvr = SrtSocketBuilder::new_listen().local_port(8876).connect().await.unwrap();
