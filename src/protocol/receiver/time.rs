        ...
        fn synchronize(drift_ts in 1u64..5_000_000) {
            ...
            for tick_ts in 1..1002 {
                ...
                if tick_ts < MAX_SAMPLES {
                    prop_assert_eq!(instant, start + tick, "the clock should not be adjusted until {} samples: tick_ts = {}", MAX_SAMPLES, tick_ts);
                }
                else if tick_ts == MAX_SAMPLES {
                    prop_assert_eq!(instant, now, "the clock should be adjusted after {} samples", MAX_SAMPLES);
                }
                else {
                    prop_assert_eq!(instant, now, "the clock should not be adjusted until the next {} samples: tick_ts = {}", MAX_SAMPLES, tick_ts);
                }
            }
