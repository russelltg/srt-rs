...
inline constexpr size_t Size(const V (&)[N]) noexcept {
  return N;
}
 ...
 * listener accepts it, while caller rejects it. In this case we have a
 short-living
 * confusion state: The connection is accepted on the listener side, and the
 listener
 ...
 * just a matter of luck: if the listener thread is lucky, it will report the
 socket
 * to accept, so epoll will signal it and accept will report it, and moreover,
 further
 * good luck on this socket would make the state check return SRTS_CONNECTED.
 Without
 * this good luck, the caller might be quick enough to reject the handshake and
 send
 * the UMSG_SHUTDOWN packet to the peer. If it gets with it before acceptance,
 it will
 ...
 * - The accepted socket was reported, and after `srt_connect` is done, it
 should turn to SRTS_BROKEN.
 *
 * This embraces both cases when the accepted socket was broken in the
 beginning, and when it was CONNECTED
 * in the beginning, but broke soon thereafter.
 *
 * This behavior is predicted and accepted - it's also the reason that setting
 ENFORCEDENC to false is
 * NOT RECOMMENDED on a listener socket that isn't intended to accept only
 connections from known callers
 * that are known to have set this flag also to false.
 *
 * In the cases C.2-C.4 it is the listener who rejects the connection, so we
 don't have an accepted socket
 * and the situation is always the same and clear in the beginning. The caller
 cannot continue with the
 * connection after listener accepted it, even if it tolerates incompatible
 password settings.
 */
...
const TestCaseNonBlocking g_test_matrix_non_blocking[] = {
    // ENFORCEDENC       |  Password           | | EPoll wait | socket_state |
    // KM State caller | listener |  caller  | listener |  connect_ret
    // accept_ret      |  ret         | event             | caller accepted |
    // caller              listener
    /*A.1 */ {{true, true},
              {s_pwd_a, s_pwd_a},
              {SRT_SUCCESS,
               ...
               {SRT_KM_S_SECURED, SRT_KM_S_SECURED}}},
    /*A.2 */
    {{true, true},
     {s_pwd_a, s_pwd_b},
     {SRT_SUCCESS,
      ...
      0,
      0,
      {SRTS_BROKEN, IGNORE_SRTS},
      {SRT_KM_S_UNSECURED, IGNORE_SRTS}}},
    /*A.3 */
    {{true, true},
     {s_pwd_a, s_pwd_no},
     {SRT_SUCCESS,
    ...
    {{true, true},
     ...
     {SRT_SUCCESS,
      ...
      SRT_EPOLL_IN,
      {SRTS_CONNECTED, SRTS_CONNECTED},
      {SRT_KM_S_UNSECURED, SRT_KM_S_UNSECURED}}},
    ...
    {{true, false},
     ...
     {SRT_SUCCESS,
      ...
      {SRT_KM_S_BADSECRET, SRT_KM_S_BADSECRET}}},
    /*B.3 */
    {{true, false},
     {s_pwd_a, s_pwd_no},
     {SRT_SUCCESS,
    ...
    {{true, false},
     ...
     {SRT_SUCCESS,
      ...
      0,
      {SRTS_CONNECTING, SRTS_BROKEN},
      {SRT_KM_S_UNSECURED, SRT_KM_S_NOSECRET}}},
    /*B.5 */
    {{true, false},
     {s_pwd_no, s_pwd_no},
     {SRT_SUCCESS,
    ...
    {{false, false},
     ...
     {SRT_SUCCESS,
      ...
      SRT_EPOLL_IN,
      {SRTS_CONNECTED, SRTS_CONNECTED},
      {SRT_KM_S_NOSECRET, SRT_KM_S_NOSECRET}}},
    /*D.5 */
    {{false, false},
     {s_pwd_no, s_pwd_no},
     {SRT_SUCCESS,
 ...
 *
 * In the cases B.2-B.4 the caller will reject the connection due to the
 * enforced encryption check of the HS response from the listener on the stage
 * of the KM response check. While the listener accepts the connection with the
 * connected state. So the caller sends UMSG_SHUTDOWN to notify the listener
 * that it has closed the connection. The accepted socket gets the SRTS_BROKEN
 * states. For these cases a special accept_ret = -2 is used, that allows the
 * accepted socket to be broken or already closed.
 *
 * In the cases C.2-C.4 it is the listener who rejects the connection, so we
 * don't have an accepted socket.
 */
const TestCaseBlocking g_test_matrix_blocking[] = {
    ...
    // caller | listener |  caller  | listener |  connect_ret         accept_ret
    // | caller                accepted |  caller              listener
    /*A.1 */ {{true, true},
              {s_pwd_a, s_pwd_a},
              {SRT_SUCCESS,
               0,
...
class TestEnforcedEncryption : public ::testing::Test {
 protected:
 ...
 protected:
  ...
  void SetUp() {
    ...
    ASSERT_NE(
        srt_setsockflag(m_caller_socket, SRTO_SENDER, &s_yes, sizeof s_yes),
        SRT_ERROR);
    ASSERT_NE(srt_setsockopt(m_caller_socket, 0, SRTO_TSBPDMODE, &s_yes,
                             sizeof s_yes),
              SRT_ERROR);
    ...
    ASSERT_NE(
        srt_setsockflag(m_listener_socket, SRTO_SENDER, &s_no, sizeof s_no),
        SRT_ERROR);
    ASSERT_NE(srt_setsockopt(m_listener_socket, 0, SRTO_TSBPDMODE, &s_yes,
                             sizeof s_yes),
              SRT_ERROR);

    // Will use this epoll to wait for srt_accept(...)
    const int epoll_out = SRT_EPOLL_IN | SRT_EPOLL_ERR;
    ASSERT_NE(srt_epoll_add_usock(m_pollid, m_listener_socket, &epoll_out),
              SRT_ERROR);
  }
 ...
 public:
  int SetEnforcedEncryption(PEER_TYPE peer, bool value) {
    const SRTSOCKET& socket =
        peer == PEER_CALLER ? m_caller_socket : m_listener_socket;
    return srt_setsockopt(socket, 0, SRTO_ENFORCEDENCRYPTION,
                          value ? &s_yes : &s_no, sizeof s_yes);
  }

  bool GetEnforcedEncryption(PEER_TYPE peer_type) {
    const SRTSOCKET socket =
        peer_type == PEER_CALLER ? m_caller_socket : m_listener_socket;
    bool optval;
    int optlen = sizeof optval;
    EXPECT_EQ(srt_getsockopt(socket, 0, SRTO_ENFORCEDENCRYPTION, (void*)&optval,
                             &optlen),
              SRT_SUCCESS);
  ...
  int SetPassword(PEER_TYPE peer_type, const std::basic_string<char>& pwd) {
    const SRTSOCKET socket =
        peer_type == PEER_CALLER ? m_caller_socket : m_listener_socket;
    return srt_setsockopt(socket, 0, SRTO_PASSPHRASE, pwd.c_str(),
                          (int)pwd.size());
  }
  ...
  int GetKMState(SRTSOCKET socket) {
    ...
    EXPECT_EQ(srt_getsockopt(socket, 0, SRTO_KMSTATE,
                             reinterpret_cast<void*>(&km_state), &opt_size),
              SRT_SUCCESS);
  ...
  int GetSocetkOption(SRTSOCKET socket, SRT_SOCKOPT opt) {
    ...
    int size = sizeof val;
    EXPECT_EQ(
        srt_getsockopt(socket, 0, opt, reinterpret_cast<void*>(&val), &size),
        SRT_SUCCESS);
  ...
  void TestConnect(TEST_CASE test_case /*, bool is_blocking*/) {
    ...
    if (is_blocking) {
      ASSERT_NE(
          srt_setsockopt(m_caller_socket, 0, SRTO_RCVSYN, &s_yes, sizeof s_yes),
          SRT_ERROR);
      ASSERT_NE(
          srt_setsockopt(m_caller_socket, 0, SRTO_SNDSYN, &s_yes, sizeof s_yes),
          SRT_ERROR);
      ASSERT_NE(srt_setsockopt(m_listener_socket, 0, SRTO_RCVSYN, &s_yes,
                               sizeof s_yes),
                SRT_ERROR);
      ASSERT_NE(srt_setsockopt(m_listener_socket, 0, SRTO_SNDSYN, &s_yes,
                               sizeof s_yes),
                SRT_ERROR);
    } else {
      ASSERT_NE(
          srt_setsockopt(m_caller_socket, 0, SRTO_RCVSYN, &s_no, sizeof s_no),
          SRT_ERROR);  // non-blocking mode
      ASSERT_NE(
          srt_setsockopt(m_caller_socket, 0, SRTO_SNDSYN, &s_no, sizeof s_no),
          SRT_ERROR);  // non-blocking mode
      ASSERT_NE(
          srt_setsockopt(m_listener_socket, 0, SRTO_RCVSYN, &s_no, sizeof s_no),
          SRT_ERROR);  // non-blocking mode
      ASSERT_NE(
          srt_setsockopt(m_listener_socket, 0, SRTO_SNDSYN, &s_no, sizeof s_no),
          SRT_ERROR);  // non-blocking mode
    }

    // Prepare input state
    const TestCase<TResult>& test = GetTestMatrix<TResult>(test_case);
    ASSERT_EQ(SetEnforcedEncryption(PEER_CALLER, test.enforcedenc[PEER_CALLER]),
              SRT_SUCCESS);
    ASSERT_EQ(
        SetEnforcedEncryption(PEER_LISTENER, test.enforcedenc[PEER_LISTENER]),
        SRT_SUCCESS);

    ASSERT_EQ(SetPassword(PEER_CALLER, test.password[PEER_CALLER]),
              SRT_SUCCESS);
    ASSERT_EQ(SetPassword(PEER_LISTENER, test.password[PEER_LISTENER]),
              SRT_SUCCESS);
    ...
    auto accepting_thread = std::thread([&] {
      ...
      // In a blocking mode we expect a socket returned from srt_accept() if the
      // srt_connect succeeded. In a non-blocking mode we expect a socket
      // returned from srt_accept() if the srt_connect succeeded, otherwise
      // SRT_INVALID_SOCKET after the listening socket is closed.
      sockaddr_in client_address;
      int length = sizeof(sockaddr_in);
      SRTSOCKET accepted_socket = -1;
      if (epoll_event == SRT_EPOLL_IN) {
        accepted_socket =
            srt_accept(m_listener_socket, (sockaddr*)&client_address, &length);
      ...
      if (accepted_socket == SRT_INVALID_SOCK) {
        std::cerr << "[T] ACCEPT ERROR: " << srt_getlasterror_str()
                  << std::endl;
      } else {
      ...
      if (accepted_socket != SRT_INVALID_SOCK &&
          expect.socket_state[CHECK_SOCKET_ACCEPTED] != IGNORE_SRTS) {
        if (m_is_tracing) {
          std::cerr
              << "EARLY Socket state accepted: "
              << m_socket_state[srt_getsockstate(accepted_socket)]
              << " (expected: "
              << m_socket_state[expect.socket_state[CHECK_SOCKET_ACCEPTED]]
              << ")\n";
          std::cerr << "KM State accepted:     "
                    << m_km_state[GetKMState(accepted_socket)] << '\n';
          std::cerr
              << "RCV KM State accepted:     "
              << m_km_state[GetSocetkOption(accepted_socket, SRTO_RCVKMSTATE)]
              << '\n';
          std::cerr
              << "SND KM State accepted:     "
              << m_km_state[GetSocetkOption(accepted_socket, SRTO_SNDKMSTATE)]
              << '\n';
        }

        // We have to wait some time for the socket to be able to process the HS
        // responce from the caller. In test cases B2 - B4 the socket is
        // expected to change its state from CONNECTED to BROKEN due to KM
        // mismatches
        do {
          std::this_thread::sleep_for(std::chrono::milliseconds(50));
        } while (!caller_done);

        // Special case when the expected state is "broken": if so, tolerate
        // every possible socket state, just NOT LESS than SRTS_BROKEN, and also
        // don't read any flags on that socket.

        if (expect.socket_state[CHECK_SOCKET_ACCEPTED] == SRTS_BROKEN) {
          EXPECT_GE(srt_getsockstate(accepted_socket), SRTS_BROKEN);
        } else {
          EXPECT_EQ(srt_getsockstate(accepted_socket),
                    expect.socket_state[CHECK_SOCKET_ACCEPTED]);
          EXPECT_EQ(GetSocetkOption(accepted_socket, SRTO_SNDKMSTATE),
                    expect.km_state[CHECK_SOCKET_ACCEPTED]);
        }

        if (m_is_tracing) {
          const SRT_SOCKSTATUS status = srt_getsockstate(accepted_socket);
          std::cerr
              << "LATE Socket state accepted: " << m_socket_state[status]
              << " (expected: "
              << m_socket_state[expect.socket_state[CHECK_SOCKET_ACCEPTED]]
              << ")\n";
        }
    ...
    if (is_blocking == false) accepting_thread.join();

    if (m_is_tracing) {
      std::cerr << "Socket state caller:   "
                << m_socket_state[srt_getsockstate(m_caller_socket)] << "\n";
      std::cerr << "Socket state listener: "
                << m_socket_state[srt_getsockstate(m_listener_socket)] << "\n";
      std::cerr << "KM State caller:       "
                << m_km_state[GetKMState(m_caller_socket)] << '\n';
      std::cerr << "RCV KM State caller:   "
                << m_km_state[GetSocetkOption(m_caller_socket, SRTO_RCVKMSTATE)]
                << '\n';
      std::cerr << "SND KM State caller:   "
                << m_km_state[GetSocetkOption(m_caller_socket, SRTO_SNDKMSTATE)]
                << '\n';
      std::cerr << "KM State listener:     "
                << m_km_state[GetKMState(m_listener_socket)] << '\n';
    }

    // If a blocking call to srt_connect() returned error, then the state is not
    // valid, but we still check it because we know what it should be. This way
    // we may see potential changes in the core behavior.
    if (is_blocking) {
      EXPECT_EQ(srt_getsockstate(m_caller_socket),
                expect.socket_state[CHECK_SOCKET_CALLER]);
    }
    // A caller socket, regardless of the mode, if it's not expected to be
    // connected, check negatively.
    ...
    EXPECT_EQ(GetSocetkOption(m_caller_socket, SRTO_RCVKMSTATE),
              expect.km_state[CHECK_SOCKET_CALLER]);

    EXPECT_EQ(srt_getsockstate(m_listener_socket), SRTS_LISTENING);
    EXPECT_EQ(GetKMState(m_listener_socket), SRT_KM_S_UNSECURED);

    if (is_blocking) {
      // srt_accept() has no timeout, so we have to close the socket and wait
      // for the thread to exit. Just give it some time and close the socket.
      std::this_thread::sleep_for(std::chrono::milliseconds(50));
      ASSERT_NE(srt_close(m_listener_socket), SRT_ERROR);
...
template <>
int TestEnforcedEncryption::WaitOnEpoll<TestResultBlocking>(
    const TestResultBlocking&) {
  return SRT_EPOLL_IN;
}

static std::ostream& PrintEpollEvent(std::ostream& os, int events,
                                     int et_events) {
  using namespace std;

  static pair<int, const char*> const namemap[] = {
      make_pair(SRT_EPOLL_IN, "R"), make_pair(SRT_EPOLL_OUT, "W"),
      make_pair(SRT_EPOLL_ERR, "E"), make_pair(SRT_EPOLL_UPDATE, "U")};
  ...
  for (int i = 0; i < N; ++i) {
    if (events & namemap[i].first) {
      os << "[";
      if (et_events & namemap[i].first) os << "^";
      os << namemap[i].second << "]";
    }
  }
...
template <>
int TestEnforcedEncryption::WaitOnEpoll<TestResultNonBlocking>(
    const TestResultNonBlocking& expect) {
  ...
  if (epoll_res == SRT_ERROR) {
    std::cerr << "Epoll returned error: " << srt_getlasterror_str() << " (code "
...
template <>
const TestCase<TestResultBlocking>&
TestEnforcedEncryption::GetTestMatrix<TestResultBlocking>(
    TEST_CASE test_case) const {
  return g_test_matrix_blocking[test_case];
}

template <>
const TestCase<TestResultNonBlocking>&
TestEnforcedEncryption::GetTestMatrix<TestResultNonBlocking>(
    TEST_CASE test_case) const {
...
const char* TestEnforcedEncryption::m_km_state[] = {
    ...
    "SRT_KM_S_SECURED   (2)",  // Stream encrypted, keying Material exchanged,
                               // decrypting ok.
    "SRT_KM_S_NOSECRET  (3)",  // Stream encrypted and no secret to decrypt
                               // Keying Material
    "SRT_KM_S_BADSECRET (4)"   // Stream encrypted and wrong secret, cannot
                               // decrypt Keying Material
};

static const char* const socket_state_array[] = {
    "IGNORE_SRTS",    "SRTS_INVALID",    "SRTS_INIT",      "SRTS_OPENED",
    "SRTS_LISTENING", "SRTS_CONNECTING", "SRTS_CONNECTED", "SRTS_BROKEN",
    "SRTS_CLOSING",   "SRTS_CLOSED",     "SRTS_NONEXIST"};

// A trick that allows the array to be indexed by -1
const char* const* TestEnforcedEncryption::m_socket_state =
    socket_state_array + 1;
...
TEST_F(TestEnforcedEncryption, PasswordLength) {
  ...
  EXPECT_EQ(SetPassword(PEER_LISTENER, std::string("proper_length")),
            SRT_SUCCESS);
#else
...
#define CREATE_TEST_CASE_BLOCKING(CASE_NUMBER, DESC)              \
  TEST_F(TestEnforcedEncryption, CASE_NUMBER##_Blocking_##DESC) { \
    TestConnect<TestResultBlocking>(TEST_##CASE_NUMBER);          \
  }

#define CREATE_TEST_CASE_NONBLOCKING(CASE_NUMBER, DESC)              \
  TEST_F(TestEnforcedEncryption, CASE_NUMBER##_NonBlocking_##DESC) { \
