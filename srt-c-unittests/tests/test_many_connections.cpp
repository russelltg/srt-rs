...
class TestConnection : public ::testing::Test {
 ...
 protected:
  ...
  void SetUp() override {
    ...
    ASSERT_GE(bind_res, 0) << "srt_bind returned " << bind_res << ": "
    ...
    std::generate(m_buf.begin(), m_buf.end(),
                  [dis, gen]() mutable { return (char)dis(gen); });

    ASSERT_NE(srt_listen(m_server_sock, NSOCK), -1);
  }

  void TearDown() override { srt_cleanup(); }
  ...
  void AcceptLoop() {
    for (;;) {
      ...
      if (acp == -1) {
        cerr << "[T] Accept error at " << m_accepted.size() << "/" << NSOCK
             << ": " << srt_getlasterror_str() << endl;
        break;
      }
...
// This test establishes multiple connections to a single SRT listener on a
// localhost port. Packets are submitted for sending to all those connections in
// a non-blocking mode. Then all connections are closed. Some sockets may
// potentially still have undelivered packets. This test tries to reproduce the
// issue described in #1182, and fixed by #1315.
TEST_F(TestConnection, Multiple) {
  ...
  for (size_t i = 0; i < NSOCK; i++) {
    ...
    int conntimeo = 60;
    srt_setsockflag(m_connections[i], SRTO_CONNTIMEO, &conntimeo,
                    sizeof conntimeo);
    ...
    int no = 0;
    ASSERT_NE(srt_setsockflag(m_connections[i], SRTO_SNDSYN, &no, sizeof no),
              -1);
  }
