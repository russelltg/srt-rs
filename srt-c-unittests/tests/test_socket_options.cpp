...
class TestSocketOptions : public ::testing::Test {
 protected:
  TestSocketOptions() {
 ...
 public:
  ...
  SRTSOCKET EstablishConnection() {
    auto accept_async = [](SRTSOCKET listen_sock) {
      ...
      const SRTSOCKET accepted_socket =
          srt_accept(listen_sock, (sockaddr*)&client_address, &length);
 ...
 protected:
  ...
  void SetUp() {
    ...
    ASSERT_EQ(srt_setsockopt(m_caller_sock, 0, SRTO_RCVSYN, &yes, sizeof yes),
              SRT_SUCCESS);  // for async connect
    ASSERT_EQ(srt_setsockopt(m_caller_sock, 0, SRTO_SNDSYN, &yes, sizeof yes),
              SRT_SUCCESS);  // for async connect

    m_listen_sock = srt_create_socket();
    ASSERT_NE(m_listen_sock, SRT_INVALID_SOCK);
    ASSERT_EQ(srt_setsockopt(m_listen_sock, 0, SRTO_RCVSYN, &yes, sizeof yes),
              SRT_SUCCESS);  // for async connect
    ASSERT_EQ(srt_setsockopt(m_listen_sock, 0, SRTO_SNDSYN, &yes, sizeof yes),
              SRT_SUCCESS);  // for async connect
  }
...
enum class RestrictionType { PREBIND = 0, PRE = 1, POST = 2 };
...
struct OptionTestEntry {
  ...
  RestrictionType
      restriction;  // TODO: consider using SRTO_R_PREBIND, etc. from core.cpp
  size_t opt_len;
  linb::any min_val;
  linb::any max_val;
  linb::any dflt_val;
...
static const size_t UDP_HDR_SIZE =
    28;  // 20 bytes IPv4 + 8 bytes of UDP { u16 sport, dport, len, csum }.
static const size_t DFT_MTU_SIZE = 1500;  // Default MTU size
static const size_t SRT_PKT_SIZE =
    DFT_MTU_SIZE - UDP_HDR_SIZE;  // MTU without UDP header

const OptionTestEntry g_test_matrix_options[] = {
    // Option ID,                Option Name |          Restriction | optlen |
    // min |       max |  default | nondefault |  invalid vals |
    // SRTO_BINDTODEVICE
    //{ SRTO_CONGESTION,      "SRTO_CONGESTION",  RestrictionType::PRE, 4,
    //"live",     "file",   "live",       "file",   {"liv", ""} },
    {SRTO_CONNTIMEO,
     "SRTO_CONNTIMEO",
     RestrictionType::PRE,
     sizeof(int),
...
void CheckGetSockOpt(const OptionTestEntry& entry, SRTSOCKET sock,
                     const ValueType& value, const char* desc) {
  ValueType opt_val;
  int opt_len = 0;
  EXPECT_EQ(srt_getsockopt(sock, 0, entry.optid, &opt_val, &opt_len),
            SRT_SUCCESS)
      << "Getting " << entry.optname
      << " returned error: " << srt_getlasterror_str();

  EXPECT_EQ(opt_val, value)
      << desc << ": Wrong " << entry.optname << " value " << opt_val;
  EXPECT_EQ(opt_len, entry.opt_len)
      << desc << "Wrong " << entry.optname << " value length";
}

typedef char const* strptr;
template <>
void CheckGetSockOpt<strptr>(const OptionTestEntry& entry, SRTSOCKET sock,
                             const strptr& value, const char* desc) {
  char opt_val[16];
  int opt_len = 0;
  EXPECT_EQ(srt_getsockopt(sock, 0, entry.optid, &opt_val, &opt_len),
            SRT_SUCCESS)
  ...
  EXPECT_EQ(strncmp(opt_val, value, min<int>(opt_len, entry.opt_len)), 0)
      << desc << ": Wrong " << entry.optname << " value " << opt_val;
  EXPECT_EQ(opt_len, entry.opt_len)
      << desc << "Wrong " << entry.optname << " value length";
}
...
template <class ValueType>
void CheckSetSockOpt(const OptionTestEntry& entry, SRTSOCKET sock,
                     const ValueType& value, int expect_return,
                     const char* desc) {
  ValueType opt_val = value;
  int opt_len = entry.opt_len;
  EXPECT_EQ(srt_setsockopt(sock, 0, entry.optid, &opt_val, opt_len),
            expect_return)
      << "Setting " << entry.optname << " to " << opt_val << " must "
      << (expect_return == SRT_SUCCESS ? "succeed" : "fail");
...
template <class ValueType>
bool CheckDefaultValue(const OptionTestEntry& entry, SRTSOCKET sock,
                       const char* desc) {
  try {
    ...
    std::cerr << entry.optname
              << " default value type: " << entry.dflt_val.type().name()
              << "\n";
    return false;
  }
...
template <class ValueType>
bool CheckSetNonDefaultValue(const OptionTestEntry& entry, SRTSOCKET sock,
                             int expected_return, const char* desc) {
  try {
    /*const ValueType dflt_val = linb::any_cast<ValueType>(entry.dflt_val);
    ...
    std::cerr << entry.optname
              << " non-default value type: " << entry.ndflt_val.type().name()
              << "\n";
    return false;
  }
...
template <class ValueType>
bool CheckMinValue(const OptionTestEntry& entry, SRTSOCKET sock,
                   const char* desc) {
  try {
    const ValueType min_val = linb::any_cast<ValueType>(entry.min_val);
    ...
    std::cerr << entry.optname
              << " min value type: " << entry.min_val.type().name() << "\n";
    return false;
  }
...
template <class ValueType>
bool CheckMaxValue(const OptionTestEntry& entry, SRTSOCKET sock,
                   const char* desc) {
  try {
    const ValueType max_val = linb::any_cast<ValueType>(entry.max_val);
    ...
    std::cerr << entry.optname
              << " max value type: " << entry.max_val.type().name() << "\n";
    return false;
  }
...
template <class ValueType>
bool CheckInvalidValues(const OptionTestEntry& entry, SRTSOCKET sock,
...
TEST_F(TestSocketOptions, DefaultVals) {
  for (const auto& entry : g_test_matrix_options) {
    ...
    if (entry.dflt_val.type() == typeid(bool)) {
      ...
      EXPECT_TRUE(
          CheckDefaultValue<const char*>(entry, m_caller_sock, test_desc));
    } else {
      FAIL() << entry.optname << ": Unexpected type "
             << entry.dflt_val.type().name();
    }
  }
}
...
TEST_F(TestSocketOptions, MaxVals) {
  ...
  for (const auto& entry : g_test_matrix_options) {
    if (entry.optid == SRTO_KMPREANNOUNCE ||
        entry.optid == SRTO_KMREFRESHRATE) {
...
TEST_F(TestSocketOptions, RestrictionBind) {
  ...
  for (const auto& entry : g_test_matrix_options) {
    ...
    const int expected_res = (entry.restriction == RestrictionType::PREBIND)
                                 ? SRT_ERROR
                                 : SRT_SUCCESS;

    if (entry.dflt_val.type() == typeid(bool)) {
      EXPECT_TRUE(CheckSetNonDefaultValue<bool>(entry, m_listen_sock,
                                                expected_res, test_desc))
          << "Sock state : " << g_socket_state[srt_getsockstate(m_listen_sock)];
    } else if (entry.dflt_val.type() == typeid(int)) {
      EXPECT_TRUE(CheckSetNonDefaultValue<int>(entry, m_listen_sock,
                                               expected_res, test_desc))
          << "Sock state : " << g_socket_state[srt_getsockstate(m_listen_sock)];
    } else if (entry.dflt_val.type() == typeid(int64_t)) {
      EXPECT_TRUE(CheckSetNonDefaultValue<int64_t>(entry, m_listen_sock,
                                                   expected_res, test_desc))
          << "Sock state : " << g_socket_state[srt_getsockstate(m_listen_sock)];
    } else {
      FAIL() << "Unexpected type " << entry.dflt_val.type().name();
...
// Check that only socket option with POST binding can be set on a listener
// socket in "listening" state.
TEST_F(TestSocketOptions, RestrictionListening) {
  StartListener();

  for (const auto& entry : g_test_matrix_options) {
    const int expected_res =
        (entry.restriction != RestrictionType::POST) ? SRT_ERROR : SRT_SUCCESS;
...
// Check that only socket option with POST binding can be set on a connected
// socket (caller and accepted).
TEST_F(TestSocketOptions, RestrictionConnected) {
  ...
  for (const auto& entry : g_test_matrix_options) {
    ...
    for (SRTSOCKET sock : {m_caller_sock, accepted_sock}) {
      const char* test_desc = sock == m_caller_sock ? "[Caller, connected]"
                                                    : "[Accepted, connected]";

      if (entry.dflt_val.type() == typeid(bool)) {
        EXPECT_TRUE(
            CheckSetNonDefaultValue<bool>(entry, sock, expected_res, test_desc))
            << test_desc << entry.optname
            << " Sock state: " << g_socket_state[srt_getsockstate(sock)];
      } else if (entry.dflt_val.type() == typeid(int)) {
        EXPECT_TRUE(
            CheckSetNonDefaultValue<int>(entry, sock, expected_res, test_desc))
            << test_desc << entry.optname
            << " Sock state: " << g_socket_state[srt_getsockstate(sock)];
      } else if (entry.dflt_val.type() == typeid(int64_t)) {
        EXPECT_TRUE(CheckSetNonDefaultValue<int64_t>(entry, sock, expected_res,
                                                     test_desc))
            << test_desc << entry.optname
            << " Sock state: " << g_socket_state[srt_getsockstate(sock)];
      } else {
...
TEST_F(TestSocketOptions, TLPktDropInherits) {
  ...
  EXPECT_EQ(srt_setsockopt(m_listen_sock, 0, SRTO_TLPKTDROP, &tlpktdrop_new,
                           sizeof tlpktdrop_new),
            SRT_SUCCESS);
  EXPECT_EQ(srt_getsockopt(m_listen_sock, 0, SRTO_TLPKTDROP, &optval, &optlen),
            SRT_SUCCESS);
  ...
  for (SRTSOCKET sock : {m_listen_sock, accepted_sock}) {
    ...
    EXPECT_EQ(srt_getsockopt(sock, 0, SRTO_TLPKTDROP, &optval, &optlen),
              SRT_SUCCESS);
...
TEST_F(TestSocketOptions, Latency) {
  ...
  int optval;
  int optlen = (int)(sizeof optval);
  EXPECT_EQ(srt_setsockopt(m_listen_sock, 0, SRTO_RCVLATENCY, &latency_a,
                           sizeof latency_a),
            SRT_SUCCESS);
  EXPECT_EQ(srt_setsockopt(m_listen_sock, 0, SRTO_PEERLATENCY, &latency_b,
                           sizeof latency_b),
            SRT_SUCCESS);

  EXPECT_EQ(srt_getsockopt(m_listen_sock, 0, SRTO_RCVLATENCY, &optval, &optlen),
            SRT_SUCCESS);
  EXPECT_EQ(optval, latency_a);
  EXPECT_EQ(
      srt_getsockopt(m_listen_sock, 0, SRTO_PEERLATENCY, &optval, &optlen),
      SRT_SUCCESS);
  ...
  // Check caller socket
  EXPECT_EQ(srt_getsockopt(m_caller_sock, 0, SRTO_RCVLATENCY, &optval, &optlen),
            SRT_SUCCESS);
  EXPECT_EQ(optval, latency_dflt);
  EXPECT_EQ(
      srt_getsockopt(m_caller_sock, 0, SRTO_PEERLATENCY, &optval, &optlen),
      SRT_SUCCESS);
  EXPECT_EQ(optval, latency_a);

  EXPECT_EQ(srt_getsockopt(accepted_sock, 0, SRTO_RCVLATENCY, &optval, &optlen),
            SRT_SUCCESS);
  EXPECT_EQ(optval, latency_a);
  EXPECT_EQ(
      srt_getsockopt(accepted_sock, 0, SRTO_PEERLATENCY, &optval, &optlen),
      SRT_SUCCESS);
  EXPECT_EQ(optval, latency_dflt);
...
TEST_F(TestSocketOptions, LossMaxTTL) {
  ...
  ASSERT_EQ(srt_setsockopt(m_listen_sock, 0, SRTO_LOSSMAXTTL, &loss_max_ttl,
                           sizeof loss_max_ttl),
            SRT_SUCCESS);
  ...
  int opt_len = 0;
  ASSERT_EQ(
      srt_getsockopt(accepted_sock, 0, SRTO_LOSSMAXTTL, &opt_val, &opt_len),
      SRT_SUCCESS);
  EXPECT_EQ(opt_val, loss_max_ttl)
      << "Wrong SRTO_LOSSMAXTTL value on the accepted socket";
  EXPECT_EQ(size_t(opt_len), sizeof opt_len)
      << "Wrong SRTO_LOSSMAXTTL value length on the accepted socket";
  ...
  ASSERT_EQ(
      srt_getsockopt(m_listen_sock, 0, SRTO_LOSSMAXTTL, &opt_val, &opt_len),
      SRT_SUCCESS);
  EXPECT_EQ(opt_val, loss_max_ttl)
      << "Wrong SRTO_LOSSMAXTTL value on the listener socket";
  EXPECT_EQ(size_t(opt_len), sizeof opt_len)
      << "Wrong SRTO_LOSSMAXTTL value length on the listener socket";
...
TEST_F(TestSocketOptions, MinInputBWWrongLen) {
  ...
  EXPECT_EQ(
      srt_getsockopt(m_listen_sock, 0, SRTO_MININPUTBW, &mininputbw, &optlen),
      SRT_ERROR);
  ...
  EXPECT_EQ(
      srt_getsockopt(m_listen_sock, 0, SRTO_MININPUTBW, &mininputbw, &optlen),
      SRT_SUCCESS)
      << "Bigger storage is allowed";
  EXPECT_EQ(optlen, (int)(sizeof mininputbw));

  EXPECT_EQ(srt_setsockopt(m_listen_sock, 0, SRTO_MININPUTBW, &mininputbw,
                           sizeof mininputbw - 1),
            SRT_ERROR);
  EXPECT_EQ(srt_getlasterror(NULL), SRT_EINVPARAM);
  EXPECT_EQ(srt_setsockopt(m_listen_sock, 0, SRTO_MININPUTBW, &mininputbw,
...
TEST_F(TestSocketOptions, MinInputBWDefault) {
  ...
  for (SRTSOCKET sock : {m_listen_sock, accepted_sock}) {
    ...
    EXPECT_EQ(srt_getsockopt(sock, 0, SRTO_MININPUTBW, &mininputbw, &optlen),
              SRT_SUCCESS);
    EXPECT_EQ(optlen, (int)(sizeof mininputbw));
    EXPECT_EQ(mininputbw, mininputbw_expected);
  }
...
TEST_F(TestSocketOptions, MinInputBWSet) {
  ...
  int64_t bw = -100;
  EXPECT_EQ(srt_setsockopt(m_listen_sock, 0, SRTO_MININPUTBW, &bw, sizeof bw),
            SRT_ERROR)
      << "Has to be a non-negative number";
  EXPECT_EQ(srt_getsockopt(m_listen_sock, 0, SRTO_MININPUTBW, &bw, &optlen),
            SRT_SUCCESS);
  ...
  for (SRTSOCKET sock : {m_listen_sock, accepted_sock}) {
    ...
    EXPECT_EQ(srt_getsockopt(sock, 0, SRTO_MININPUTBW, &bw, &optlen),
              SRT_SUCCESS);
...
TEST_F(TestSocketOptions, MinInputBWRuntime) {
  ...
  EXPECT_EQ(srt_setsockopt(accepted_sock, 0, SRTO_MININPUTBW, &bw, sizeof bw),
            SRT_SUCCESS);
  EXPECT_EQ(srt_getsockopt(accepted_sock, 0, SRTO_MININPUTBW, &bw, &optlen),
            SRT_SUCCESS);
  EXPECT_EQ(bw, mininputbw);

  bw = 0;
  EXPECT_EQ(srt_setsockopt(accepted_sock, 0, SRTO_INPUTBW, &bw, sizeof bw),
            SRT_SUCCESS);
  EXPECT_EQ(srt_getsockopt(accepted_sock, 0, SRTO_INPUTBW, &bw, &optlen),
            SRT_SUCCESS);
  EXPECT_EQ(bw, 0);

  EXPECT_EQ(srt_setsockopt(accepted_sock, 0, SRTO_MAXBW, &bw, sizeof bw),
            SRT_SUCCESS);
  EXPECT_EQ(srt_getsockopt(accepted_sock, 0, SRTO_MAXBW, &bw, &optlen),
            SRT_SUCCESS);
  EXPECT_EQ(bw, 0);
...
TEST_F(TestSocketOptions, StreamIDWrongLen) {
  char buffer[MAX_SID_LENGTH + 135];
  for (size_t i = 0; i < sizeof buffer; ++i) buffer[i] = 'a' + i % 25;

  EXPECT_EQ(srt_setsockopt(m_caller_sock, 0, SRTO_STREAMID, buffer,
                           MAX_SID_LENGTH + 1),
            SRT_ERROR);
  EXPECT_EQ(srt_getlasterror(NULL), SRT_EINVPARAM);
}
...
TEST_F(TestSocketOptions, StreamIDOdd) {
  ...
  EXPECT_EQ(srt_setsockopt(m_caller_sock, 0, SRTO_STREAMID, sid_odd.c_str(),
                           sid_odd.size()),
            SRT_SUCCESS);
  ...
  EXPECT_EQ(
      srt_getsockopt(m_caller_sock, 0, SRTO_STREAMID, &buffer, &buffer_len),
      SRT_SUCCESS);
  ...
  // Check accepted socket inherits values
  for (size_t i = 0; i < sizeof buffer; ++i) buffer[i] = 'a';
  buffer_len = (int)(sizeof buffer);
  EXPECT_EQ(
      srt_getsockopt(accepted_sock, 0, SRTO_STREAMID, &buffer, &buffer_len),
      SRT_SUCCESS);
  EXPECT_EQ(size_t(buffer_len), sid_odd.size());
  EXPECT_EQ(strlen(buffer), sid_odd.size());
...
TEST_F(TestSocketOptions, StreamIDEven) {
  ...
  EXPECT_EQ(srt_setsockopt(m_caller_sock, 0, SRTO_STREAMID, sid_even.c_str(),
                           sid_even.size()),
            SRT_SUCCESS);
...
TEST_F(TestSocketOptions, StreamIDAlmostFull) {
  ...
  for (size_t i = 0; i < MAX_SID_LENGTH - 2; ++i) sid_amost_full += 'x';
  ...
  EXPECT_EQ(srt_setsockopt(m_caller_sock, 0, SRTO_STREAMID,
                           sid_amost_full.c_str(), sid_amost_full.size()),
            SRT_SUCCESS);
...
TEST_F(TestSocketOptions, StreamIDFull) {
  // 12 characters = 4*3, that is, aligned to 4
  string sid_full;
  for (size_t i = 0; i < MAX_SID_LENGTH; ++i) sid_full += 'x';
  ...
  EXPECT_EQ(srt_setsockopt(m_caller_sock, 0, SRTO_STREAMID, sid_full.c_str(),
                           sid_full.size()),
            SRT_SUCCESS);
...
TEST_F(TestSocketOptions, StreamIDLenListener) {
  ...
  EXPECT_EQ(srt_setsockopt(m_listen_sock, 0, SRTO_STREAMID,
                           stream_id_13.c_str(), stream_id_13.size()),
            SRT_SUCCESS);

  char buffer[648];
  int buffer_len = sizeof buffer;
  EXPECT_EQ(
      srt_getsockopt(m_listen_sock, 0, SRTO_STREAMID, &buffer, &buffer_len),
      SRT_SUCCESS);
  ...
  for (SRTSOCKET sock : {m_caller_sock, accepted_sock}) {
    for (size_t i = 0; i < sizeof buffer; ++i) buffer[i] = 'a';
    buffer_len = (int)(sizeof buffer);
    EXPECT_EQ(srt_getsockopt(sock, 0, SRTO_STREAMID, &buffer, &buffer_len),
              SRT_SUCCESS);
    EXPECT_EQ(buffer_len, 0)
        << (sock == accepted_sock ? "ACCEPTED" : "LISTENER");
  }
