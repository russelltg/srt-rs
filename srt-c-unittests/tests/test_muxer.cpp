...
class TestMuxer : public ::testing::Test {
 protected:
  TestMuxer() {
 ...
 protected:
  ...
  void SetUp() {
    ...
    int yes = 1;
    int no = 0;
    ASSERT_NE(srt_setsockopt(m_caller_sock, 0, SRTO_SNDSYN, &no, sizeof no),
              SRT_ERROR);  // for async connect
    ASSERT_NE(srt_setsockflag(m_caller_sock, SRTO_SENDER, &yes, sizeof yes),
              SRT_ERROR);
    ASSERT_NE(
        srt_setsockopt(m_caller_sock, 0, SRTO_TSBPDMODE, &yes, sizeof yes),
        SRT_ERROR);
 ...
 public:
  void ClientThread() {
    ...
    // sockets.
    {
      ...
      ASSERT_NE(
          srt_epoll_wait(
              m_client_pollid, read, &rlen, write, &wlen,
...
TEST_F(TestMuxer, IPv4_and_IPv6) {
  ...
  ASSERT_NE(
      srt_setsockopt(m_listener_sock_ipv4, 0, SRTO_RCVSYN, &no, sizeof no),
      SRT_ERROR);  // for async connect
  ASSERT_NE(
      srt_setsockopt(m_listener_sock_ipv4, 0, SRTO_TSBPDMODE, &yes, sizeof yes),
      SRT_ERROR);
  ...
  sa.sin_port = htons(m_listen_port);
  ASSERT_EQ(inet_pton(AF_INET, "127.0.0.1", &sa.sin_addr), 1);
  ASSERT_NE(srt_bind(m_listener_sock_ipv4, (sockaddr*)&sa, sizeof sa),
            SRT_ERROR);
  ...
  ASSERT_EQ(
      srt_setsockflag(m_listener_sock_ipv6, SRTO_IPV6ONLY, &yes, sizeof yes),
      0);
  ASSERT_NE(srt_bind(m_listener_sock_ipv6, (sockaddr*)&sa_v6, sizeof(sa_v6)),
            SRT_ERROR);
  ...
  {  // wait for connection from client
    ...
    ASSERT_NE(srt_epoll_wait(
                  m_server_pollid, read, &rlen, write, &wlen,
                  -1,  // -1 is set for debuging purpose.
                       // in case of production we need to set appropriate value
                  0, 0, 0, 0),
              SRT_ERROR);
