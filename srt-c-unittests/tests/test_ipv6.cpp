...
class TestIPv6 : public ::testing::Test {
 protected:
  int yes = 1;
  int no = 0;
 ...
 public:
  void ClientThread(int family, const std::string& address) {
    ...
    const int connect_res =
        srt_connect(m_caller_sock, (sockaddr*)&sa, sizeof sa);
    EXPECT_NE(connect_res, SRT_ERROR)
        << "srt_connect() failed with: " << srt_getlasterror_str();
    if (connect_res == SRT_ERROR) srt_close(m_listener_sock);
  ...
  void ShowAddress(std::string src, const sockaddr_any& w) {
    ...
    std::cout << src << ": " << w.str() << " (" << fam[w.family()] << ")"
              << std::endl;
  }
  ...
  sockaddr_any DoAccept() {
    ...
    EXPECT_NE(accepted_sock, SRT_INVALID_SOCK)
        << "accept() failed with: " << srt_getlasterror_str();
...
TEST_F(TestIPv6, v6_calls_v6) {
  ...
  ASSERT_EQ(srt_setsockflag(m_listener_sock, SRTO_IPV6ONLY, &yes, sizeof yes),
            0);
...
TEST_F(TestIPv6, v6_calls_v4) {
  ...
  std::thread client(&TestIPv6::ClientThread, this, AF_INET6,
                     "0::FFFF:127.0.0.1");
