...
class TestConnectionTimeout : public ::testing::Test {
 protected:
 ...
 * about SRT_EPOLL_OUT | SRT_EPOLL_ERR events on the socket calling
 * srt_epoll_wait(...). The test expects a connection timeout to happen within
 * the time, set with SRTO_CONNTIMEO (500 ms). The expected behavior is to
 * return from srt_epoll_wait(...)
 *
...
TEST_F(TestConnectionTimeout, Nonblocking) {
  ...
  EXPECT_EQ(srt_getsockopt(client_sock, 0, SRTO_CONNTIMEO, &conn_timeout,
                           &conn_timeout_len),
            SRT_SUCCESS);
  ...
  EXPECT_EQ(
      srt_setsockopt(client_sock, 0, SRTO_CONNTIMEO, &connection_timeout_ms,
                     sizeof connection_timeout_ms),
      SRT_SUCCESS);

  const int yes = 1;
  const int no = 0;
  ASSERT_EQ(srt_setsockopt(client_sock, 0, SRTO_RCVSYN, &no, sizeof no),
            SRT_SUCCESS);  // for async connect
  ASSERT_EQ(srt_setsockopt(client_sock, 0, SRTO_SNDSYN, &no, sizeof no),
            SRT_SUCCESS);  // for async connect
  ASSERT_EQ(srt_setsockopt(client_sock, 0, SRTO_TSBPDMODE, &yes, sizeof yes),
            SRT_SUCCESS);
  ASSERT_EQ(srt_setsockflag(client_sock, SRTO_SENDER, &yes, sizeof yes),
            SRT_SUCCESS);
  ...
  // Socket readiness for connection is checked by polling on WRITE allowed
  // sockets.
  {
    int rlen = 2;
    SRTSOCKET read[2];
    ...
    const chrono::steady_clock::time_point chrono_ts_start =
        chrono::steady_clock::now();

    // Here we check the connection timeout.
    // Epoll timeout is set 100 ms greater than socket's TTL
    EXPECT_EQ(
        srt_epoll_wait(pollid, read, &rlen, write, &wlen,
    ...
    const chrono::steady_clock::time_point chrono_ts_end =
        chrono::steady_clock::now();
    const auto delta_ms = chrono::duration_cast<chrono::milliseconds>(
                              chrono_ts_end - chrono_ts_start)
                              .count();
    // Confidence interval border : +/-80 ms
    EXPECT_LE(delta_ms, connection_timeout_ms + 80)
        << "Timeout was: " << delta_ms;
...
TEST_F(TestConnectionTimeout, BlockingLoop) {
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    const auto delta_ms = chrono::duration_cast<chrono::milliseconds>(
                              chrono::steady_clock::now() - chrono_ts_start)
                              .count();
    // Confidence interval border : +/-200 ms
    EXPECT_LE(delta_ms, connection_timeout_ms + 200)
        << "Timeout was: " << delta_ms;
    EXPECT_GE(delta_ms, connection_timeout_ms - 200)
        << "Timeout was: " << delta_ms;

    const int error_code = srt_getlasterror(nullptr);
    EXPECT_EQ(error_code, SRT_ENOSERVER);
    if (error_code != SRT_ENOSERVER) {
      cerr << "Connection attempt no. " << i << " resulted with: " << error_code
