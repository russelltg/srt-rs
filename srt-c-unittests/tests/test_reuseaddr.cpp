...
sockaddr_any CreateAddr(const std::string& name, unsigned short port,
                        int pref_family = AF_INET) {
  using namespace std;
  ...
  sockaddr_any result;
  addrinfo fo = {0, pref_family, 0, 0, 0, 0, NULL, NULL};
...
std::string GetLocalIP() {
  std::cout
      << "!!!WARNING!!!: GetLocalIP not supported, test FORCEFULLY passed\n";
  return "";
}
#else
struct IfAddr {
  ifaddrs* handle;
  IfAddr() { getifaddrs(&handle); }

  ~IfAddr() { freeifaddrs(handle); }
};
...
void clientSocket(std::string ip, int port, bool expect_success) {
  ...
  ASSERT_NE(srt_setsockopt(m_client_sock, 0, SRTO_SNDSYN, &no, sizeof no),
            SRT_ERROR);  // for async connect
  ASSERT_NE(srt_setsockflag(m_client_sock, SRTO_SENDER, &yes, sizeof yes),
            SRT_ERROR);

  ASSERT_NE(srt_setsockopt(m_client_sock, 0, SRTO_TSBPDMODE, &yes, sizeof yes),
            SRT_ERROR);
  ...
  if (expect_success) {
...

    {
      ...
      EXPECT_NE(srt_epoll_wait(
                    client_pollid, read, &rlen, write, &wlen,
...
void serverSocket(std::string ip, int port, bool expect_success) {
  ...
  ASSERT_NE(srt_setsockopt(m_bindsock, 0, SRTO_RCVSYN, &no, sizeof no),
            SRT_ERROR);  // for async connect
  ASSERT_NE(srt_setsockopt(m_bindsock, 0, SRTO_TSBPDMODE, &yes, sizeof yes),
            SRT_ERROR);
  ...
  {  // wait for connection from client
    ...
    ASSERT_NE(
        srt_epoll_wait(
            server_pollid, read, &rlen, write, &wlen,
  ...
  srt_close(m_client_sock);  // cannot close m_client_sock after srt_sendmsg
                             // because of issue in api.c:2346
...
TEST(ReuseAddr, SameAddr2) {
  ...
  if (localip == "") return;  // DISABLE TEST if this doesn't work.
...
TEST(ReuseAddr, Wildcard) {
  ...
  std::cout << "!!!WARNING!!!: On Windows connection to localhost this way "
               "isn't possible.\n"
