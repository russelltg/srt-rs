...
TEST(Transmission, FileUpload) {
  ...
  for (int port = 5000; port <= 5555; ++port) {
    ...
    ASSERT_TRUE(bind_res == SRT_EINVOP)
        << "Bind failed not due to an occupied port. Result " << bind_res;
  }
  ...
  {
    std::cout << "WILL CREATE source file with size=" << filesize << " (= 7 * "
              << optval << "[sndbuf])\n";
  ...
  auto client = std::thread([&] {
    ...
    const SRTSOCKET accepted_sock =
        srt_accept(sock_lsn, (sockaddr*)&remote, &len);
    ...
    std::ofstream copyfile("file.target",
