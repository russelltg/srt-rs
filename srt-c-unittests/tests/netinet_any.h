...
struct sockaddr_any {
  ...
  struct SysLenWrapper {
    ...
    ~SysLenWrapper() { backwriter = syslen; }
  };

  // Usage:
  //    ::accept(lsn_sock, sa.get(), &sa.syslen());
  SysLenWrapper syslen() { return SysLenWrapper(len); }
  ...
  bool empty() const {
    ...
    if (sa.sa_family == AF_INET) {
      ...
      isempty = (sin6.sin6_port == 0 && memcmp(&sin6.sin6_addr, &in6addr_any,
  ...
  len_t size() const { return size(sa.sa_family); }
  ...
  const void* get_addr() const {
    if (sa.sa_family == AF_INET) return &sin.sin_addr.s_addr;

    if (sa.sa_family == AF_INET6) return &sin6.sin6_addr;
  ...
  struct Equal {
    bool operator()(const sockaddr_any& c1, const sockaddr_any& c2) {
      if (c1.family() != c2.family()) return false;
      ...
      return c1.sin.sin_port == c2.sin.sin_port && c1.equal_address(c2);
    }
  };
  ...
  struct EqualAddress {
    bool operator()(const sockaddr_any& c1, const sockaddr_any& c2) {
      ...
      if (c1.sa.sa_family == AF_INET6) {
        return memcmp(&c1.sin6.sin6_addr, &c2.sin6.sin6_addr,
                      sizeof(in6_addr)) == 0;
      }
  ...
  bool isany() const {
    if (sa.sa_family == AF_INET) return sin.sin_addr.s_addr == INADDR_ANY;
  ...
  std::string str() const {
    if (family() != AF_INET && family() != AF_INET6) return "unknown:0";
...
template <>
struct sockaddr_any::TypeMap<AF_INET> {
  typedef sockaddr_in type;
};
template <>
struct sockaddr_any::TypeMap<AF_INET6> {
  typedef sockaddr_in6 type;
};

template <>
inline sockaddr_any::TypeMap<AF_INET>::type& sockaddr_any::get<AF_INET>() {
  return sin;
}
template <>
inline sockaddr_any::TypeMap<AF_INET6>::type& sockaddr_any::get<AF_INET6>() {
  return sin6;
}
